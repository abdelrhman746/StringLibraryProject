...
enum enMainMenueList{ShowClientList=1,AddNewClient,DeleteClient,UpdateClientInfo,FindClient,Exit};
...
void AddNewClientCard(vector<stBankClientData>vClients)
{
    ...
    {
        cout << "\nClient Wit [" << BankClientData.AccountNumber<<"] already exists, Enter Another Account Number" << endl;
        getline(cin >> ws, BankClientData.AccountNumber);
    }
